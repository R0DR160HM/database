...
- Function `transaction`, together with the new `TransactionError` type, now flattens its result. No more `Ok(Ok(result))` or, event worse, `Ok(Error(reason))`, just `Ok(result)` or `Error(reason)`
...
- Function `upsert` created, allowing to - finally - save a value with your own ID, instead of relying on an randomly generated one.
